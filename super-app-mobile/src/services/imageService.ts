...
export const imageService = {
  async pickImage(): Promise<{ uri: string; name: string; type: string } | null> {
    try {
      // 1. Xin quyền truy cập thư viện ảnh trên Native
      if (Platform.OS !== 'web') {
      ...
      try {
        ...
        console.warn('[ImageService] manipulateAsync failed, using original uri:', manipErr);
        const filename = originalUri.split('/').pop() || 'avatar.jpg';
        return {
          uri: originalUri,
          name: filename,
          type: 'image/jpeg',
